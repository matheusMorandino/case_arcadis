...
class ConsolidadorTabela:
    @staticmethod
    def normaliza_nome_colunas_ezmtp(df_ezmtp: pd.DataFrame):
        """
    ...
    @staticmethod
    def avalia_integridade_input(df_cadastro: pd.DataFrame, df_ezmtp: pd.DataFrame):
        """
    ...
    @staticmethod
    def verificar_nan_no_dataframe(df: pd.DataFrame, sheet_nome: str):
        """
    ...
    def ezmtp_formatador(self, df_ezmtp: pd.DataFrame):
        """
    ...
    @staticmethod
    def cadastro_formatador(df_cadastro: pd.DataFrame):
        """
    ...
    @staticmethod
    def extrai_valores_ezmt(df_ezmtp: pd.DataFrame):
        """
    ...
    @staticmethod
    def adicionando_identificadores_medicao(df_extracao: pd.DataFrame, df_ezmtp: pd.DataFrame):
        """
    ...
    @staticmethod
    def adicona_unidade_param(df_extracao: pd.DataFrame):
        """
    ...
    def consolida_arquivo(self, excel_path: str):
        ...
        cadastro_sheet_df = self.cadastro_formatador(excel_dataframes["Cadastro"])
        ezmtp_sheet_df = self.ezmtp_formatador(excel_dataframes["EZMtp"])

        self.avalia_integridade_input(cadastro_sheet_df, ezmtp_sheet_df)
        self.verificar_nan_no_dataframe(cadastro_sheet_df, sheet_nome="Cadastro")
        self.verificar_nan_no_dataframe(cadastro_sheet_df, sheet_nome="EZMtp")

        # Processando os valores para o formato necessário
        ezmtp_sheet_df = self.normaliza_nome_colunas_ezmtp(ezmtp_sheet_df)

        sheet_extracao = self.extrai_valores_ezmt(ezmtp_sheet_df)
        sheet_extracao = self.adicona_unidade_param(sheet_extracao)
        df_consolidado = self.adicionando_identificadores_medicao(df_extracao=sheet_extracao,
